...
BEST_MODEL_PATH = CURRENT_DIR / "best_model_cross_pose.pth"
...
def main() -> None:
	...
	required_columns = {"GroupID", "Subject", "Pose"}
	missing_columns = required_columns - set(labels_df.columns)
	if missing_columns:
		raise ValueError(f"CSV 缺少必要列: {sorted(missing_columns)}")

	# 严格执行跨姿态/位置泛化测试：Pose==0 用于训练，Pose==1 用于测试
	samples_df = labels_df.loc[:, ["GroupID", "Subject", "Pose"]].copy()
	...
	train_df = samples_df[samples_df["Pose"] == 0].loc[:, ["file_path", "Subject"]]
	test_df = samples_df[samples_df["Pose"] == 1].loc[:, ["file_path", "Subject"]]

	if train_df.empty:
		raise ValueError("训练集为空，请检查 Pose == 0 的数据是否存在。")
	if test_df.empty:
		raise ValueError("测试集为空，请检查 Pose == 1 的数据是否存在。")
